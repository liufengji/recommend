package com.victor.bussiness.service;

import com.victor.bussiness.model.recom.Recommendation;
import com.victor.bussiness.model.request.*;
...
public class RecommenderService {
    ...
    private List<Recommendation> findContentBasedMoreLikeThisRecommendations(int mid, int maxItems) {
        MoreLikeThisQueryBuilder query = QueryBuilders.moreLikeThisQuery(/*new String[]{"name", "descri", "genres", "actors", "directors", "tags"},*/
                new MoreLikeThisQueryBuilder.Item[]{new MoreLikeThisQueryBuilder.Item("index", "movie", String.valueOf(mid))});
    ...
    private List<Recommendation> findStreamRecs(int uid,int maxItems){
        String result = null;
        result = jdbcTemplate.query("select * from streamrecs where uid=" + uid, new ResultSetExtractor<String>() {
            @Override
            public String extractData(ResultSet resultSet) throws SQLException, DataAccessException {
                String result = null;
                if (resultSet.next()) {
    ...
    private List<Recommendation> findContentBasedSearchRecommendations(String text, int maxItems) {
        ...
        return parseESResponse(esClient.prepareSearch().setIndices("index").setTypes("movie").setQuery(query).setSize(maxItems).execute().actionGet());
    ...
    public List<Recommendation> getHotRecommendations(HotRecommendationRequest request) {
        ...
        recs = jdbcTemplate.query("select * from ratemoterecnetlymovies ORDER BY yeahmonth DESC limit " + request.getSum(), new RowMapper<Recommendation>() {
            @Override
    ...
    public List<Recommendation> getRateMoreRecommendations(RateMoreRecommendationRequest request) {
        List<Recommendation> recs = null;
        recs = jdbcTemplate.query("select * from ratemoterecnetlymovies ORDER BY count DESC limit " + request.getSum(), new RowMapper<Recommendation>() {
            @Override
            public Recommendation mapRow(ResultSet resultSet, int i) throws SQLException {
                Recommendation rec = new Recommendation();
                rec.setMid(resultSet.getInt("mid"));
    ...
    public List<Recommendation> getContentBasedGenresRecommendations(SearchRecommendationRequest request) {
        ...
        return parseESResponse(esClient.prepareSearch().setIndices("index").setTypes("movie").setQuery(query).setSize(request.getSum()).execute().actionGet());
    }
