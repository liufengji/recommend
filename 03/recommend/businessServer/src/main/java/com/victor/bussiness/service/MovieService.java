...
import com.victor.bussiness.model.domain.Movie;
import com.victor.bussiness.model.recom.Recommendation;
import com.victor.bussiness.model.request.NewRecommendationRequest;
...
public class MovieService {
    ...
    public List<Movie> getMovies(List<Integer> mids) {
        ...
        movies = jdbc.query("select * from " + Constants.DB_MOVIES() + " where mid in (:param)",paramMap, new RowMapper<Movie>() {
            @Override
            public Movie mapRow(ResultSet resultSet, int i) throws SQLException {
                ...
                Double score = jdbcTemplate.query("select * from " + Constants.DB_MOVIE_AVERAGE_SCORE() + " where mid=" + movie.getMid(), new ResultSetExtractor<Double>() {
                    @Override
    ...
    public Movie findByMID(int mid) {
        Movie movie = null;
        movie = jdbcTemplate.query("select *from " + Constants.DB_MOVIES() + " where mid=" + mid, new ResultSetExtractor<Movie>() {
            @Override
    ...
    public List<Movie> getMyRateMovies(int uid) {
        ...
        movies = jdbcTemplate.query("select * from " + Constants.DB_RATINGS() + " where uid=" + uid, new RowMapper<Integer>() {
            @Override
    ...
    public List<Movie> getNewMovies(NewRecommendationRequest request) {
        List<Movie> movies = null;
        movies = jdbcTemplate.query("select * from " + Constants.DB_MOVIES() + " ORDER BY issue DESC LIMIT " + request.getSum(), new RowMapper<Movie>() {
            @Override
            public Movie mapRow(ResultSet resultSet, int i) throws SQLException {
                Movie movie = new Movie();
                movie.setMid(resultSet.getInt("mid"));
